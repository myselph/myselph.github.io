function initFrictionSim() {
    ...
    // Stairs Demo
    (function () {
        const chart = setupChart('stairsFrictionCanvas');
        ...
        startStopButton.addEventListener('click', () => {
            ...
            const friction = parseFloat(document.querySelector('#stairs input[name=friction]').value) || 0.2;
            ...
            const body = new Body([[1, -1], [-1, -1], [-1, 1], [1, 1]], [x0, y0], theta0, 1 / mass, 1 / moi, [vx0, vy0], vAng0);
            body.addForce([0, -mass * 9.81]);
            world.addBody(body);
            for (let i = -1; i < 16; i += 2) {
                world.addBody(new Body([[10, -1], [-10, -1], [-10, 1], [10, 1]], [-3 - i, -2 + i], 0, 0, 0, [0, 0], 0));
            }
    ...
    // Domino Demo
    (function () {
        const chart = setupChart('dominoFrictionCanvas');
        let simulationRunning = false;
        ...
        startStopButton.addEventListener('click', () => {
            ...
            const friction = parseFloat(document.querySelector('#domino input[name=friction]').value) || 0.2;
...
if (typeof Chart !== 'undefined') {
    ...
    window.addEventListener('load', initFrictionSim);
}
