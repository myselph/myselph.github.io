...
- The set of all constraints are usually coupled: a body that shares two constraints with two other bodies, such as a link in the middle of a chain, is affected bny and affects both connected links. However, this is neglected when solving for pairwise constraints individually, and satisfying one constraint will usually violate the other constraint(s).
...
I did not add parameters to this simulation, because using links with less mass leads to jitter. I simply went to 1/4th of the previous time step size - 4ms. I suppose that rope simulations with a larger number of links always lead to problems; if one uses not sequential impulses but springs to simulate it, one gets a stiff system of equations. There's probably a way to take better care of it though; but keep in mind a rope is a rather extreme application of joints because of the many couplings. Other, similarly complex applications of joint chains like ragdolls are treated as a special case as far as I know.
